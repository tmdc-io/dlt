...
class IcebergLoadFilesystemJob(TableFormatLoadFilesystemJob):
    def run(self) -> None:
        ...
        if write_disposition == "merge" and table is not None:
            logger.debug(
                f"pyiceberg: merge table={self.load_table_name}"
            )
            merge_iceberg_table(
                table=table,
                data=arrow_rbr,
            ...
            logger.debug(
                f"pyiceberg: {write_disposition} table={self.load_table_name}"
            )
            write_iceberg_table(
                table=table,
                data=arrow_rbr,
        ...
        if table_format in ("delta", "iceberg"):
            ...
            if ReferenceFollowupJobRequest.is_reference_job(file_path):
                if table_format == "delta":
                    ...
                    # Use Spark engine when iceberg_spark_config is set on the destination
                    # config, or when the DLT_ICEBERG_WRITE_ENGINE=spark env var is present.
                    use_spark = bool(
                        getattr(self.config, "iceberg_spark_config", None)
                        or os.environ.get("DLT_ICEBERG_WRITE_ENGINE", "").lower() == "spark"
                    )
                    if use_spark:
                        from dlt.destinations.impl.filesystem.iceberg_spark_job import (
                            IcebergSparkLoadFilesystemJob,
                        )

                        return IcebergSparkLoadFilesystemJob(file_path)
                    else:
                        # Default: PyIceberg
