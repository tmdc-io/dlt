...
def test_add_config_to_env(environment: Dict[str, str]) -> None:
    ...
    add_config_to_env(c, ("nilus",))
    # must contain dlt prefix everywhere, INSTRUMENTED section taken from key and DLT_TEST taken from password
    assert (
        environment.items()
        >= {
            "NILUS__DEFAULT": "BUBA",
            "NILUS__INSTRUMENTED__HEAD": "h",
            "NILUS__INSTRUMENTED__TUBE": '["tu","u","be"]',
            "NILUS__INSTRUMENTED__HEELS": "he",
            "NILUS__DLT_TEST__PASSWORD": "PASS",
        }.items()
    )
    # no dlt
    environment.clear()
    ...
    add_config_to_env(c_s, ("nilus",))
    assert environment["NILUS__CREDENTIALS__QUERY__ENCRYPT"] == "yes"
    assert environment["NILUS__CREDENTIALS__QUERY__TRUSTSERVERCERTIFICATE"] == "yes"
